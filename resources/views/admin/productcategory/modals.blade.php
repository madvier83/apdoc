...
?>
<form  method="POST" id="editCategoryForm" action="{{ route('admin.productcategory.update',['id'=>$data->id]) }}" enctype="multipart/form-data">
    @csrf
    {{ method_field('PUT') }}
                ...
                <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" required name="parent_id">
                                ...
                                @endif
                                @foreach(ProdukCategory::where('parent_id', null)->get() as $item)
                                    @if ($data->parent_id != null)
                                        ...
                                        @endif
                                    @else
                                        @if(ProdukCategory::where('parent_id','=','id'))
                                        <option value="{{$item->id}}">{{$item->name}}</option>
                                        @endif
                                    @endif
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="seed" value="{{ $data->seed}}" min="1" name="seed" required>
                        </div>
                </div>
                </div>
            </div>
